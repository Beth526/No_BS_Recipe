...
def add_new_lines_instructions(string,html_translation_dict):
    ...
    for key, value in translation_dict.items():
        string = string.replace(key, value)
    string=html.unescape(string)
    starts = [(m.start()) for m in re.finditer('Cook |Fry |Bake |Heat |Season |Combine |Add |Mix |Blend |Stir |Shake |Bring |Remove |Sprinkle |Sift |Seperate |Place |Top |Pour |Fill |Spoon |Make |Place |Cut |Chop |Strain |Garnish |Serve |In a |Peel |Puree |Wrap |Roll |Preheat |Pre-heat |Melt |Marinate |Simmer |Steam |Prick |Process |With a |Store |Freeze |Refridgerate |Microwave |Toast |Grill |Chill |Sauté |Saute |Cover |Allow |Defrost |Use |Boil |Chill |Rub |Swirl |Slice |Sear |Soak |Heat |While ',string)]
    string = list(string)
    offset=0
    for s in starts:
        string.insert(s+offset,'  \n  \n '+str(offset+1)+') ')
